...
import type { Severity, ValidationResult } from '@seo-solver/types/validate';
...
import { createValidationPipeline } from '../pipeline/create-validation-pipeline.js';
...
export async function validatePage(input: ExtractedPageInput, options: ValidatePageOptions = {}) {
...
function getAllowedValidatorTypes(validators: ValidatePageOptions['validators']): ReadonlySet<string> | undefined {
  if (!validators) {
    return undefined;
  }

  return new Set(validators.filter((validatorType) => validatorType !== '*'));
}
